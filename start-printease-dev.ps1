...
if (-not $NoDocker) {
    $dockerComposeFile = Join-Path $mpayDir "docker-compose.dev.yml"
    if (-not (Test-Path $dockerComposeFile)) {
        throw "Docker Compose dev file not found: $dockerComposeFile"
    }
    if ($DryRun) {
        ...
        try {
            # Use & operator for compatibility with older PowerShell versions
        ...
        Write-Host "[OK] Docker(mpay-dev) started"
    }
}
...
Write-Host "Development startup completed. Default services started: Docker(mpay-dev) + Backend + FRP + UniApp"
